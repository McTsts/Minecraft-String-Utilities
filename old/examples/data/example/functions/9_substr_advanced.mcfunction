...
data modify storage string:io queue append value {string:"This is an example string!",callback:{command:"function example:9_substr_advanced",id:1}}
execute unless score $callback string matches 1.. run tellraw @a ["[#09] Input: ",{"storage":"string:io","nbt":"queue[-1].string","interpret":false}]
function string:call

execute if score $callback string matches 1 run tellraw @a ["[#09] Char Array: ",{"nbt":"out","storage":"string:io","interpret":false}]
