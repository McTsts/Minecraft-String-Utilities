...
execute unless score $callback string matches 0.. run tellraw @a ["[#17] Input: ",{"storage":"string:io","nbt":"queue[-1].string","interpret":false}]
function string:call

# Output
execute if score $callback string matches 1 run tellraw @a ["[#17] Output: ",{"storage":"string:io","nbt":"out","interpret":true}]