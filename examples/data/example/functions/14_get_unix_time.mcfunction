...
execute unless score $callback string matches 0.. run data modify storage string:io temp set value {string:"",callback:{command:"function example:14_get_unix_time",id:1}}
execute unless score $callback string matches 0.. run data modify storage string:io temp.string set from block 0 0 0 Owner.Properties.textures[0].Value
